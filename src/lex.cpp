...
#include "c.h"
Value lexBuffer;
bool judgeCharacter(char c) {					//数字、a-z大小写
...
int lex(Value& lexBuffer){
	while (true) {
		...
		case '5':case '6':case '7':case '8':case '9': {
			char* temp = codePos - 1;		//整数
			int num = 0;
			while (*temp >= '0' && *temp <= '9') {
				num = num * 10 + *temp - '0';
				temp++;
			}
			if (*temp == '.' || *temp == 'e' || *temp == 'E')goto REAL;
			codePos = temp;
			lexBuffer.i = num;
			return NUM;
		}REAL: {								//实数
		...
		id: {									//变量
			int cur = 0;
			lexBuffer.s = (char*)malloc(100 * sizeof(char));
			while (!judgeCharacter(*(codePos + cur - 1))) { 
				lexBuffer.s[cur++] = *(codePos + cur -1);
			}lexBuffer.s[cur] = '\0';
